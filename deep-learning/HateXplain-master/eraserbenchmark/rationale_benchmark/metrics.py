...
def score_classifications(instances: List[dict], annotations: List[Annotation], docs: Dict[str, List[str]], aopc_thresholds: List[float]) -> Dict[str, float]:
    ...
    print(key_to_instances)
    truth = []
    predicted = []
    print([ann.annotation_id for ann in annotations])
    for ann in annotations:
        # print(ann)
        truth.append(label_to_int[ann.classification])

        if ann.annotation_id not in key_to_instances:
            predicted.append(label_to_int['non-toxic'])
            continue

        inst = key_to_instances[ann.annotation_id]
        # print(inst)
        if inst['classification'] == 'hatespeech':
            predicted.append(label_to_int['toxic'])
        else:
            predicted.append(label_to_int['non-toxic'])
    ...
    print(truth)
    print(predicted)
    classification_scores = classification_report(truth, predicted, output_dict=True, target_names=labels, digits=2)
