...
dict_data_folder={
      '2':{'data_file':'Data/dataset.json','class_label':'Data/classes.npy'},
      '3':{'data_file':'Data/dataset.json','class_label':'Data/classes.npy'}
}
...
#     device = torch.device("cpu")
    ...
    if(extra_data_path!=None):
        ...
        # temp_read_new=temp_read[temp_read['post_id'].isin(post_id_dict['test']) & (temp_read['final_label'].isin(['toxic']))]
        temp_read_new=temp_read[temp_read['post_id'].isin(post_id_dict['test'])]
        test_data=get_test_data(temp_read_new,params,message='text')
...
#         m = nn.Softmax(dim=1)
        ...
        if(params['bert_tokens']):
            ...
            attention_vectors= outputs[0].detach().cpu().numpy()
...
#             continue
        ...
        temp["classification_scores"]={"hatespeech":logits[0],"normal":logits[1]}
