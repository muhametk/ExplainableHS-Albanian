...
and_rules_eval_df = pd.DataFrame(columns=columns)
...
for rule, nr_matches, rule_eval in tqdm(all_rules_eval):
    ...
    and_rules_eval_df = and_rules_eval_df.append(row, ignore_index=True)

and_rules_eval_df_sorted = and_rules_eval_df.sort_values(by='nr_matches', ascending=False)
and_rules_eval_df_sorted.to_csv('and_rules_eval_df_sorted.csv')